...
public class Base_Pathfinding: MonoBehaviour {
			...
			if (target != null) {
				{
					...
					if (targetNode == targetPath [0])
						continue;
					targetPath = FindPath (seeker.position, targetNode);
				}
			}
			yield return new WaitForSeconds(Random.Range(0,1f)*Time.deltaTime);
